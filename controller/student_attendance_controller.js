...
class attandance{
    ...
    postatt(req, res){
        ...
        if (req.body._id == '')
        insertRecordatt(req, res); // insert data function
        else
        updateRecordatt(req, res); // update data function
   ...
   deleteatt(req, res){
    Att.findByIdAndRemove(req.params.id, (err, doc) => {
        ...
        else { console.log('Error in att delete :' + err); }
    }).lean();
   }
    ...
    att.save((err, doc)=>{
     ...
     res.redirect('/att');
        ...
        if (err.name == 'ValidationError') {
            ...
            console.log('Error during attendance record insertion : ' + err);
    }
    })
}
...
function updateRecordatt(req, res) {
    Att.findOneAndUpdate({ _id: req.body._id }, req.body, { new: true }, (err, doc) => {
